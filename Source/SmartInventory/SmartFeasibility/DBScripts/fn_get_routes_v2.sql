...
BEGIN
	...
	select T.start_vid, T.end_vid, least((ST_Distance(ST_GeomFromText('POINT(' || p_source || ')', 4326), ST_StartPoint(single_geom), true) +
		...
		ST_Distance(ST_GeomFromText('POINT(' || p_destination || ')', 4326), ST_EndPoint(st_reverse(single_geom)), true))) as outside_length, st_astext(ST_StartPoint(single_geom)), st_astext(ST_EndPoint(single_geom)) 
		INTO v_start_vid_frwd, v_end_vid_frwd, v_length_frwd, v_start_point, v_end_point
	FROM(
	...
	FROM pgr_dijkstra(
	'SELECT id, source, target, cost, reverse_cost FROM new_poc_data2', v_start_vid, v_end_vid,
	directed:= True) as pt
	JOIN new_poc_data2_noded nd ON pt.edge = nd.old_id 
	JOIN new_poc_data2 main ON pt.edge = main.id
	order by path_seq;

    ...
    internal class UploadTempMPOD : UploadExcel
    {
        ...
        public override ErrorMessage UploadData(DataTable dataTable, UploadSummary summary)
        {
            try
            {
                ...
                {
                    List<TempMPOD> lstEntity = DataValidator.GetObjectList<TempMPOD>(dataTable, summary);
            ...
            {
                return tempToMainTable.UpdateStatusAndGetError("UploadTempMPOD", "Upload Data", ex, summary);
            }
        }
        private ErrorMessage ParallelInsert(int TotalBatchCount, List<TempMPOD> lstMPOD, UploadSummary summary)
        {
            ErrorMessage errorMessage = new ErrorMessage();
            List<List<TempMPOD>> lstChunks = lstMPOD.ToChunks(TotalBatchCount).ToList();
            ...
            BLTempMPOD blTempMPOD = new BLTempMPOD();
            int batchCount = 0;

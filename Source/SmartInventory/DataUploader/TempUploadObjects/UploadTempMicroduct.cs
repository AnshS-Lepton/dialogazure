...
{
    internal class UploadTempMicroduct : UploadExcel, IUploadKML
    {
        ...
        public override ErrorMessage UploadData(DataTable dataTable, UploadSummary summary)
        {
            try
            {
                ...
                {
                    List<TempMicroduct> lstEntity = DataValidator.GetObjectList<TempMicroduct>(dataTable, summary);
            ...
            {
                return tempToMainTable.UpdateStatusAndGetError("UploadTempMicroduct", "Upload Data", ex, summary);
            }
        }
        private ErrorMessage ParallelInsert(int TotalBatchCount, List<TempMicroduct> lstObj, UploadSummary summary)
        {
            ErrorMessage errorMessage = new ErrorMessage();
            List<List<TempMicroduct>> lstChunks = lstObj.ToChunks(TotalBatchCount).ToList();
            ...
            BLTempMicroduct blTempObj = new BLTempMicroduct();
            int batchCount = 0;

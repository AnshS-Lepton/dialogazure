...
{
    internal class UploadTempGipipe : UploadExcel, IUploadKML
    {
        ...
        public UploadTempGipipe()
        {
            kmlUploader = new KMLUploader();
            tempToMainTable = new TempToMainTable();
            tempToMainTable.UploadStatusEventTemp += NotifyUpdateStatus;
        ...
        public override ErrorMessage UploadData(DataTable dataTable, UploadSummary summary)
        {
            try
            {
                ...
                {
                    List<TempGipipe> lstEntity = DataValidator.GetObjectList<TempGipipe>(dataTable,summary);
            ...
            {
                return tempToMainTable.UpdateStatusAndGetError("UploadTempDuct", "Upload Data", ex, summary);
            }
        }
        private ErrorMessage ParallelInsert(int TotalBatchCount, List<TempDuct> lstObj, UploadSummary summary)
        {
            ErrorMessage errorMessage = new ErrorMessage();
            List<List<TempDuct>> lstChunks = lstObj.ToChunks(TotalBatchCount).ToList();
            ...
            BLTempDuct blTempObj = new BLTempDuct();
            int batchCount = 0;
        ...
        private ErrorMessage ParallelInsert(int TotalBatchCount, List<TempGipipe> lstObj, UploadSummary summary)
        {
            ErrorMessage errorMessage = new ErrorMessage();
            List<List<TempGipipe>> lstChunks = lstObj.ToChunks(TotalBatchCount).ToList();
            var options = new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount };
            object lockObj = new object();
            int currentStatus = 0;
            int CurrentValue = 0;
            DateTime dtStarts = DateTimeHelper.Now;
            var exceptions = new ConcurrentQueue<Exception>();
            BLTempGipipe blTempObj = new BLTempGipipe();
            int batchCount = 0;
            string EstimatedRemainingTime;
            ParallelLoopResult result = Parallel.ForEach(lstChunks, options, (item) =>
            {

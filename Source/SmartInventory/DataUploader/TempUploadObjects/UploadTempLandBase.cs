   ...
   internal class UploadTempLandBase : UploadExcel
    {
        ...
        public override ErrorMessage UploadData(DataTable dataTable, UploadSummary summary)
        {
            try
            {
                ...
                {
                    List<TempLandBase> lstEntity = DataValidator.GetObjectList<TempLandBase>(dataTable, summary);
            ...
            {
                return tempToMainTable.UpdateStatusAndGetError("UploadTempLandBase", "Upload Data", ex, summary);

            }
        }
        private ErrorMessage ParallelInsert(int TotalBatchCount, List<TempLandBase> lstLandBase, UploadSummary summary)
        {
            ErrorMessage errorMessage = new ErrorMessage();
            List<List<TempLandBase>> lstChunks = lstLandBase.ToChunks(TotalBatchCount).ToList();
            ...
            BusinessLogics.BLTemp.BLTempLandBase blTempLandBase = new BusinessLogics.BLTemp.BLTempLandBase();
            int batchCount = 0;

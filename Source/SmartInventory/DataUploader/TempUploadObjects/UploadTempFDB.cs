    ...
    internal class UploadTempFDB : UploadExcel
    {
        ...
        public override ErrorMessage UploadData(DataTable dataTable, UploadSummary summary)
        {
            try
            {
                ...
                {
                    List<TempFDB> lstEntity = DataValidator.GetObjectList<TempFDB>(dataTable,summary);
            ...
            {
                return tempToMainTable.UpdateStatusAndGetError("UploadTempFDB", "Upload Data", ex, summary);
            }
        }
        private ErrorMessage ParallelInsert(int TotalBatchCount, List<TempFDB> lstFDB, UploadSummary summary)
        {
            ErrorMessage errorMessage = new ErrorMessage();

            List<List<TempFDB>> lstChunks = lstFDB.ToChunks(TotalBatchCount).ToList();
            ...
            BLTempFDB blTempFDB = new BLTempFDB();
            int batchCount = 0;

...
namespace DataUploader
{
    ...
    internal class UploadTempRow : UploadExcel, IUploadKML
    {
        ...
        public override ErrorMessage UploadData(DataTable dataTable, UploadSummary summary)
        {
            try
            {
                ...
                {
                    List<TempRow> lstEntity = DataValidator.GetObjectList<TempRow>(dataTable, summary);
            ...
            {
                return tempToMainTable.UpdateStatusAndGetError("UploadTempRow", "Upload Data", ex, summary);
            }
        }
        private ErrorMessage ParallelInsert(int TotalBatchCount, List<TempRow> lstObj, UploadSummary summary)
        {
            ErrorMessage errorMessage = new ErrorMessage();
            List<List<TempRow>> lstChunks = lstObj.ToChunks(TotalBatchCount).ToList();
            ...
            BLTempRow blTempObj = new BLTempRow();
            int batchCount = 0;

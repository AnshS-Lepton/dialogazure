    ...
    internal class UploadTempBDB : UploadExcel
    {
        ...
        public override ErrorMessage UploadData(DataTable dataTable, UploadSummary summary)
        {
            try
            {
                ...
                {
                    List<TempBDB> lstEntity = DataValidator.GetObjectList<TempBDB>(dataTable, summary);
            ...
            {
                return tempToMainTable.UpdateStatusAndGetError("UploadTempBDB", "Upload Data", ex, summary);

            }
        }
        private ErrorMessage ParallelInsert(int TotalBatchCount, List<TempBDB> lstBDB, UploadSummary summary)
        {
            ErrorMessage errorMessage = new ErrorMessage();
            List<List<TempBDB>> lstChunks = lstBDB.ToChunks(TotalBatchCount).ToList();

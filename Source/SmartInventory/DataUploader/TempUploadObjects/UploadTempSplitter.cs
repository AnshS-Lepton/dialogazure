    ...
    internal sealed class UploadTempSplitter : UploadExcel
    {
        ...
        public override ErrorMessage UploadData(DataTable dataTable, UploadSummary summary)
        {
            try
            {
                ...
                {
                    List<TempSplitter> lstEntity = DataValidator.GetObjectList<TempSplitter>(dataTable, summary);
            ...
            {
                return tempToMainTable.UpdateStatusAndGetError("UploadTempSplitter", "Upload Data", ex, summary);
            }
        }
        private ErrorMessage ParallelInsert(int TotalBatchCount, List<TempSplitter> lstTempSplitter, UploadSummary summary)
        {
            ErrorMessage errorMessage = new ErrorMessage();
            List<List<TempSplitter>> lstChunks = lstTempSplitter.ToChunks(TotalBatchCount).ToList();
            ...
            BLTempSplitter blTempSplitter = new BLTempSplitter();
            ...
            ParallelLoopResult result = Parallel.ForEach(lstChunks, options, (item) =>
            {
                try
                {
                    ...
                    lock (lockObj)
                    {
                        ...
                        summary.status_message = "Uploading..." + CurrentValue + "% Remaining Time: " + EstimatedRemainingTime; bLDataUploader.UpdateStatus(summary);
                        if (CurrentValue == 100)

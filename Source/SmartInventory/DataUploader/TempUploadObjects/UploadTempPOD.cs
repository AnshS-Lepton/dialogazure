    ...
    internal class UploadTempPOD : UploadExcel
    {
        ...
        public override ErrorMessage UploadData(DataTable dataTable, UploadSummary summary)
        {
            try
            {
                ...
                {
                    List<TempPOD> lstEntity = DataValidator.GetObjectList<TempPOD>(dataTable, summary);
            ...
            {
                return tempToMainTable.UpdateStatusAndGetError("UploadTempPOD", "Upload Data", ex, summary);

            }
        }
        private ErrorMessage ParallelInsert(int TotalBatchCount, List<TempPOD> lstPole, UploadSummary summary)
        {
            ErrorMessage errorMessage = new ErrorMessage();

            List<List<TempPOD>> lstChunks = lstPole.ToChunks(TotalBatchCount).ToList();
            ...
            BLTempPOD blTempPOD = new BLTempPOD();
            int batchCount = 0;

...
{
    internal class UploadTempTrench : UploadExcel, IUploadKML
    {
        ...
        public override ErrorMessage UploadData(DataTable dataTable, UploadSummary summary)
        {
            try
            {
                ...
                {
                    List<TempTrench> lstEntity = DataValidator.GetObjectList<TempTrench>(dataTable, summary);
            ...
            {
                return tempToMainTable.UpdateStatusAndGetError("UploadTempTrench", "Upload Data", ex, summary);
            }
        }
        private ErrorMessage ParallelInsert(int TotalBatchCount, List<TempTrench> lstObj, UploadSummary summary)
        {
            ErrorMessage errorMessage = new ErrorMessage();
            List<List<TempTrench>> lstChunks = lstObj.ToChunks(TotalBatchCount).ToList();
            ...
            BLTempTrench blTempObj = new BLTempTrench();
            int batchCount = 0;

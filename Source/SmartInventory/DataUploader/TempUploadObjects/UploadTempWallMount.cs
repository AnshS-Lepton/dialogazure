    ...
    internal class UploadTempWallMount : UploadExcel
    {
        ...
        public override ErrorMessage UploadData(DataTable dataTable, UploadSummary summary)
        {
            try
            {
                ...
                {
                    List<TempWallMount> lstEntity = DataValidator.GetObjectList<TempWallMount>(dataTable, summary);
            ...
            {
                return tempToMainTable.UpdateStatusAndGetError("UploadTempWallMount", "Upload Data", ex, summary);
            }
        }
        private ErrorMessage ParallelInsert(int TotalBatchCount, List<TempWallMount> lstWallMount, UploadSummary summary)
        {
            ErrorMessage errorMessage = new ErrorMessage();
            List<List<TempWallMount>> lstChunks = lstWallMount.ToChunks(TotalBatchCount).ToList();
            ...
            BLTempWallMount blTempWallMount = new BLTempWallMount();
            int batchCount = 0;

    ...
    internal class UploadTempVault : UploadExcel
    {
        ...
        public override ErrorMessage UploadData(DataTable dataTable, UploadSummary summary)
        {
            try
            {
                ...
                {
                    List<TempVault> lstEntity = DataValidator.GetObjectList<TempVault>(dataTable, summary);
            ...
            {
                return tempToMainTable.UpdateStatusAndGetError("UploadTempVault", "Upload Data", ex, summary);
            }
        }
        private ErrorMessage ParallelInsert(int TotalBatchCount, List<TempVault> lstVault, UploadSummary summary)
        {
            ErrorMessage errorMessage = new ErrorMessage();
            List<List<TempVault>> lstChunks = lstVault.ToChunks(TotalBatchCount).ToList();
            ...
            BLTempVault blTempVault = new BLTempVault();
            int batchCount = 0;

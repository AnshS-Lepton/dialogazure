...
{
    public class DADTSIntegration
    {
        private readonly AppDbContext _context;
        private readonly IConfiguration _configuration;
        private readonly ApiSettings _apiSettings;

        public DADTSIntegration(IConfiguration configuration)
        {
            //var optionsBuilder = new DbContextOptionsBuilder<AppDbContext>();
            //optionsBuilder.UseNpgsql("Host=192.168.1.40; Port=5014; Database=Smartinventory_Dev; Username=postgres; Password=Lepton@#2022; CommandTimeout=600;");
        ...
        public int EntryLogInProcessSummary()
        {
            try
            {
                ...
                var processSummary = new ProcessSite
                {
                    process_start_time = DateTime.UtcNow,
                    created_by = 1, // Set this to the appropriate user ID
                ...
                return processSummary.process_id; // Return the generated process_id
            }
            catch (Exception ex)
            {
                // Log the exception or rethrow
                Console.WriteLine($"Error occurred: {ex.Message}");
                WriteLog.WriteLogFile($"Error occurred: {ex.Message}");
                throw;
            }
        }
        ...
        public void ExitLogInProcessSummary(int processID)
        {
            ...
            var existingRecord = _context.ProcessSites
                .FirstOrDefault(ps => ps.process_id == processID);

            if (existingRecord != null)
            {
                // Update the fields as necessary
                existingRecord.process_end_time = DateTime.UtcNow;
        ...
        public void SaveSitesList(List<SiteDetails> siteList, int processID)
        {
            try
            {
                foreach (var site in siteList)
                {
                    ...
                    _context.ProcessSiteLists.Add(processSiteList);
                }
        ...
        {
            var siteIds = _context.ProcessSiteLists
        ...
        {
            // Find the existing record based on process_id and site_id
            var existingRecord = _context.ProcessSiteLists
                .FirstOrDefault(ps => ps.process_id == processID && ps.site_id == site_id);

            if (existingRecord != null)
            {
                // Update the fields as necessary
                existingRecord.error_message = message; // Update site name
                existingRecord.is_valid = false; // Set is_valid to true or your specific logic
        ...
        public void SaveSiteDetails(SiteAttributes objsite, int userId, int progressID)
        {
            ...
            var existingRecord = _context.SiteDetilsMain.Select(ps => new
            {
                ps.site_id,
    ...
    .FirstOrDefault(ps => ps.site_id == objsite.site_id);
            ...
            var siteDetails = new SiteAttributes
            {
                ...
                on_air_date = objsite.on_air_date.ToUniversalTime(),
                removed_date = objsite.removed_date.ToUniversalTime(),
            ...
            {
                siteDetails.status_updated_on = objsite.status_updated_on.Value.ToUniversalTime();
            }
            ...
            else
            {
                ...
                objIn.eGeom = geom;
                var regionprovice = GetRegionProvinceDetailAsync(objIn);
            ...
            _context.SiteDetils.Add(siteDetails);
        ...
        public NetworkCodeDetail GetNetworkCodeDetail(NetworkCodeIn objIn)
        {
            ...
            SELECT * FROM fn_get_network_code(@etype, @gtype, @parent_sysid, @parent_etype, @geometry)";
                ...
                var result = _context.Set<NetworkCodeDetail>()
                    .FromSqlRaw(query,
        ...
        public InRegionProvince GetRegionProvinceDetailAsync(NetworkCodeIn objIn)
        {
            ...
            SELECT * FROM fn_getregionprovince(@geometry, @gtype)";
                ...
                var result = _context.Set<InRegionProvince>()
                    .FromSqlRaw(query,
                        new NpgsqlParameter("geometry", objIn.eGeom),
                        new NpgsqlParameter("gtype", objIn.gType))
                    .ToList();
        ...
        public ProcessSiteOutput SaveSiteDetilsInMainTable(int processID)
        {
            ...
            SELECT * FROM fn_process_site_details(@process_id_input)";
                ...
                // Execute the query using raw SQL and map it to NetworkCodeDetail
                var result = _context.Set<ProcessSiteOutput>()
                    .FromSqlRaw(query,
                        new NpgsqlParameter("process_id_input", processID))
                    .ToList();
        ...
        public void EditEntityGeometry(EditGeomIn objgeom)
        {
            try
            {
                var query = @"
        ...
        public void SaveEntityGeom(InputGeom objgeom)
        {
            try
            {
                var query = @"
        ...
        public void UpdateNetworkandGeomDetails(int progressID)
        {
            try
            {
                var siteIds = _context.SiteDetils
                ...
                foreach (var sites in siteIds)
                {
                    ...
                    var existingRecord = _context.SiteDetilsMain
                    ...
                    if (existingRecord != null && sites.is_new_entity)
                    {
                        ...
                        objIn.gType = GeometryType.Point.ToString();
                        objIn.eGeom = geom;
                        var networkDetils = GetNetworkCodeDetail(objIn);

    ...
    var action = {
        ...
        newModel: function () {
            ...
            return {
                ...
                db_border_width: 0,
                network_status: 'P',
                fms_network_id: undefined,
                is_view_enabled: true,
                text_orientation: 'LR',
                port_status: 'Vacant',
                port_status_color: '#A9A9A9',
                port_status_id: '1',
                model_color_id: '',
                border_color: '',
                ...
                network_id: '',
                short_network_id: '',
                is_internal_connectivity_enabled:false
            };
        },
        ...
        linearId: function (list) {
            ...
            for (let i = 0; i < list.length; i++) {
                let children = list.filter(x=>x.parent == list[i].id);
        ...
        getSelectedParent: function (id, parentType) {
            let index = action.getIndex(id);
            if (index == -1) return undefined;
            if (_workArea[index].model_id == parentType)
                return _workArea[index];
            if (_workArea[index].parent == -1)
                return undefined;

            return action.getSelectedParent(_workArea[index].parent, parentType);
        },
        getSizeLimitByChildren: function (id) {
            let limit = {
                x: { min: 0, max: 0 },
                y: { min: 0, max: 0 }
            };
            ...
            if (children && children.length) {
                for (let i = 0; i < children.length; i++) {
                    if (limit.x.min > action.modelPositionX(children[i])) {
                        limit.x.min = action.modelPositionX(children[i]);
                    }
                    if (limit.y.min > action.modelPositionY(children[i])) {
                        limit.y.min = action.modelPositionY(children[i]);
                    }
                    if (limit.x.max < action.modelPositionX(children[i]) + children[i].width) {
                        limit.x.max = action.modelPositionX(children[i]) + children[i].width;
                    }
                    if (limit.y.max < action.modelPositionY(children[i]) + children[i].height) {
                        limit.y.max = action.modelPositionY(children[i]) + children[i].height;
                    }
                }
        ...
        newModelCount: function () {
            let count = 0;
            count = _workArea.filter(x => x.db_id == 0 && x.parent > 0).length;
            return count;
        },

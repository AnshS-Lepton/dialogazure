var polygonProtoType = function () {
    ...
    EditPath.prototype.createLines = function () {
        ...
        for (var position = 0, i = 0; i < lineLengths.length; i++) {
            ...
            //line.setMap(map);
            var line = si.createLine(newLinePath, 10);
            line.setMap(si.map);
            // Add events
    ...
    this.deleteProtoType = function () {
        delete google.maps.Polygon.prototype.editLines;
        delete google.maps.Polygon.prototype.editable;
        delete google.maps.Polygon.prototype.setEditable;
        delete google.maps.Polygon.prototype.highlightColor;
        delete google.maps.Polygon.prototype.generateEditLines;
        delete google.maps.Polygon.prototype.lineSize;
        delete google.maps.Polygon.prototype.getEditable
        delete google.maps.Polygon.prototype.setOptions;
    }
    this.clearEditableLines = function () {
        if (si.allLines.length > 0) {
            for (var i = 0; i < si.allLines.length; i++) {
                var _lines = si.allLines[i];
                for (var j = 0; j < _lines.length; j++) {
                    _lines[j].setMap(null);
                }                
            }
            si.allLines = [];
        }
    }
    this.getFinalGeom = function () {
        var _fullPath = [];
        var _finalGeom = '';
        if (si.allLines.length > 1) {
            _finalGeom = _finalGeom + 'MULTIPOLYGON(';
            for (var i = 0; i < si.allLines.length; i++) {
                _finalGeom = _finalGeom + '((';
                var _lines = si.allLines[i];
                _fullPath = [];
                for (var j = 0; j < _lines.length; j++) {
                    _lines[j].getPath().getArray().forEach(function (item, index) { _fullPath.push(item.lng() + ' ' + item.lat()) });
                }
                _finalGeom = _finalGeom + _fullPath.join(',') + ')),';
            }
            _finalGeom = _finalGeom.slice(0, -1) + ')';
        } else {
            for (var i = 0; i < si.allLines.length; i++) {
                var _lines = si.allLines[i];
                for (var j = 0; j < _lines.length; j++) {

...
function DistanceWidget(opt_options) {
    ...
    if (!this.get('position')) {
        this.set('position', si.map.getCenter());
    }
    ...
    google.maps.event.addListener(marker, 'dragend', function (e) {
        ...
        getAOIDetail(si.distanceWidget);
        fadeOuter(si.distanceWidget.position, me.get('distance'))
    });
...
RadiusWidget.prototype.addSizer_ = function () {
    ...
    google.maps.event.addListener(sizer, 'dragend', function () {
        ...
        si.map.fitBounds(bnd);
        me.set('active', false);
        fadeOuter(me.get('center'), me.get('distance'))
        //getAOIDetail(si.distanceWidget);
        closehvrpopup();
        $('#end_point_buffer').val((me.get('distance') * 1000).toFixed(2));
        networkdata.autoPlanningEndPoint();
    });
};
...
function revGeoCodeCntr(marker) {
    ...
    si.geocoder.geocode({ 'latLng': pos }, function (results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
            if (results[1]) {
                $('#poi_address').empty().text(results[1].formatted_address);
            }
        }
    });
};
...
function getPointFromLatLng(_latLng) {
    var topRight = si.map.getProjection().fromLatLngToPoint(si.map.getBounds().getNorthEast());
    var bottomLeft = si.map.getProjection().fromLatLngToPoint(si.map.getBounds().getSouthWest());
    var scale = Math.pow(2, si.map.getZoom());
    var worldPoint = si.map.getProjection().fromLatLngToPoint(_latLng);

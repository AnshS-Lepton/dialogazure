    ...
    function _dataRenderDefault(input) {
        ...
        wrapped_list.find('tbody').find('tr').each(function () {
            ...
            that.on('mouseover', function (e) {
               ...
               //  
                /// start region create marker for auto planning
             
                var latlong = e.currentTarget.lastElementChild.innerText;

                var endlat = parseFloat(latlong.split(',')[0]);
                var endlong = parseFloat(latlong.split(',')[1]);
                endLatlng = { lat: endlat, lng: endlong };
                var southWest = new google.maps.LatLng(endlat, endlong);

                networkdata.HighlightPointEntityOnMap(endLatlng, 'Content/images/dwnArrow.png');
              //  networkdata.fitElementOnMap(networkdata.NetworkStartPoint, latlong);
                si.map.setCenter(southWest);
                /// end region create marker for auto planning

...
GRANT ALL ON FUNCTION public.fn_backbone_save_plan(bool, bool, varchar, float8, int4, varchar, varchar, varchar, int4, varchar, varchar) TO public;
...
ALTER FUNCTION public.fn_backbone_draft_network(bool, bool, varchar, int4, varchar, varchar, varchar, varchar, varchar, varchar, float8, float8, float8, varchar, varchar, float8, float8, bool) OWNER TO postgres;
GRANT ALL ON FUNCTION public.fn_backbone_draft_network(bool, bool, varchar, int4, varchar, varchar, varchar, varchar, varchar, varchar, float8, float8, float8, varchar, varchar, float8, float8, bool) TO postgres;
...
select t1.entity_type,t1.qty ::text ,itm.cost_per_unit ,itm.service_cost_per_unit ,
 ((t1.qty * itm.cost_per_unit) + (t1.qty * itm.service_cost_per_unit)) as total_cost  from (select 'Duct' as entity_type ,count(*) as qty from backbone_plan_network_details bpnd where created_by = p_user_id and plan_id = p_plan_id and entity_type in ('Manhole') and p_iscreateduct = true and system_id not in (v_skip_systemid)  group by entity_type) t1
  ...
  select t1.entity_type,t1.qty ::text ,itm.cost_per_unit ,itm.service_cost_per_unit ,
 ((t1.qty * itm.cost_per_unit) + (t1.qty * itm.service_cost_per_unit)) as total_cost  from (select 'Trench' as entity_type ,count(*) as qty from backbone_plan_network_details bpnd where created_by = p_user_id and plan_id = p_plan_id and entity_type in ('Manhole') and  p_iscreatetrench = true and system_id not in (v_skip_systemid)  group by entity_type) t1
  join item_template_master itm on itm.category_reference = t1.entity_type where lower(specification) ='generic' 
...
FROM layer_details 
WHERE upper(layer_name) IN ('POLE', 'MANHOLE', 'CABLE', 'SPLICECLOSURE','POD','DUCT','TRENCH');
...
v_calculated_loop_length := v_loop_length;
    ...
    sql :=  'SELECT
     ...
     SUM(service_cost_per_unit) AS total_service_cost_per_unit,
...
(round((measured_length)::numeric, 2) * service_cost_per_unit +  cost_per_unit * round((measured_length)::numeric, 2))as amount
 from (select (select layer_title from layer_details where layer_name ilike ''loop'')::character varying as entity_type ,''Line''::character varying as geom_type,COALESCE(COST_PER_UNIT,0)as COST_PER_UNIT,COALESCE(SERVICE_COST_PER_UNIT,0) as SERVICE_COST_PER_UNIT,'||v_loop_entity_count||' as length_qty,'||v_calculated_loop_length||' as measured_length  from att_details_cable att LEFT JOIN VENDOR_MASTER VM ON ATT.VENDOR_ID = VM.ID
LEFT JOIN AUDIT_ITEM_TEMPLATE_MASTER ITEM ON ITEM.AUDIT_ID=ATT.AUDIT_ITEM_MASTER_ID where source_ref_id='''||p_plan_id||'''  GROUP BY

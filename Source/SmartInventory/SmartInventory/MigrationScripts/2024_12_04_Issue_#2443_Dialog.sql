-- DROP FUNCTION public.fn_validate_geom_update(int4, varchar, varchar, int4, varchar, int4, varchar, int4);

CREATE OR REPLACE FUNCTION public.fn_validate_geom_update(p_system_id integer, p_geom_type character varying, p_entity_type character varying, p_userid integer, p_longlat character varying, p_bld_buffer integer, p_source_ref_type character varying, p_source_ref_id integer)
...
if COALESCE(p_source_ref_type,'')!='' and p_source_ref_id > 0 then
if exists(select 1 from polygon_master where system_id = p_source_ref_id and Upper(entity_type)='NETWORK_TICKET')then
if exists(select 1 from polygon_master pm where system_id = p_source_ref_id and Upper(entity_type)='NETWORK_TICKET' and
ST_Intersects(pm.sp_geometry,case when(lower(p_entity_type) = 'polygon' or lower(p_entity_type)= 'circle') then (select st_geomfromtext('POLYGON(('||p_longlat||'))',4326)) else
case when lower(p_entity_type) = 'line' then (select St_Geomfromtext('linestring('||p_longlat||')',4326)) else (select St_Geomfromtext('Point('||p_longlat||')',4326)) end end))then
		V_MESSAGE := 'Success'; 
		V_STATUS:=true;
	else
...
select province_id into v_province_id from att_details_networktickets where ticket_id=p_source_ref_id ;
if(v_province_id = 0)then
	V_MESSAGE := 'Success'; 
	V_STATUS:=true;
else
if exists(select 1 from province_boundary where id=v_province_id)then
if exists(select 1 from province_boundary pb where id=v_province_id and ST_Intersects(pb.sp_geometry,case when(lower(p_entity_type) = 'polygon' or lower(p_entity_type)= 'circle') then(select st_geomfromtext('POLYGON(('||p_longlat||'))',4326)) else
case when lower(p_entity_type) = 'line' then (select St_Geomfromtext('LINESTRING('||p_longlat||')',4326))
 else (select St_Geomfromtext('Point('||p_longlat||')',4326)) end end))then
		V_MESSAGE := 'Success'; 
		V_STATUS:=true;
	else
		V_MESSAGE := 'Entity can not be created out side the assigned ticket area. Please contact your administrator !';
...
THEN	
	...
	if((select ST_Equals(v_old_start_geom,v_new_start_geom))=false) AND EXISTS(SELECT 1 FROM CONNECTION_INFO WHERE ((SOURCE_SYSTEM_ID=p_system_id AND UPPER(SOURCE_ENTITY_TYPE)='CABLE') OR  DESTINATION_SYSTEM_ID=p_system_id AND UPPER(DESTINATION_ENTITY_TYPE)='CABLE') and is_cable_a_end=true)
	then
...
-- THEN
-- 	RETURN QUERY SELECT FALSE  AS STATUS, (V_LAYER_TITLE||'[SI_GBL_GBL_GBL_GBL_049]')::CHARACTER VARYING AS MESSAGE;--location can not be changed, as it is associated with some entity!
END IF;
...
-------------------------------------------------------------------------------------------------------------------------------------
-- DROP FUNCTION public.fn_validate_geom_update(int4, varchar, varchar, int4, varchar, int4, varchar, varchar);
...
------------------------------------------------------------------------------------------------
-- DROP FUNCTION public.fn_validate_geom_update(int4, varchar, varchar, int4, varchar, int4);

CREATE OR REPLACE FUNCTION public.fn_validate_geom_update(p_system_id integer, p_geom_type character varying, p_entity_type character varying, p_userid integer, p_longlat character varying, p_bld_buffer integer)
 RETURNS TABLE(status boolean, message character varying)
 LANGUAGE plpgsql
AS $function$
...
--EXECUTE 'select case when count(1)>0 then  true else false end  from '||V_LAYER_TABLE||' ent inner join associate_entity_info asso on 
--(asso.associated_system_id=ent.system_id and upper(asso.associated_entity_type)=UPPER('''||P_ENTITY_TYPE||'''))
--or (asso.entity_system_id=ent.system_id and upper(asso.entity_type)=UPPER('''||P_ENTITY_TYPE||'''))
 -- where ent.system_id='||p_system_id||' and  asso.is_termination_point=false' into V_IS_ASSOCIATED;

	IF COALESCE(V_LAYER_MAPPING.PARENT_LAYER_NAME,'')!='' AND (UPPER(V_LAYER_MAPPING.CHILD_LAYER_NAME)!='PROVINCE')
	THEN
	-- v_Query='SELECT ENT.* FROM '||V_LAYER_TABLE||' ENT
-- 	INNER JOIN(
-- 	SELECT PM.SYSTEM_ID,LM.* FROM VW_LAYER_MAPPING LM
...
-- 	
-- 	) a
-- 	ON ENT.PARENT_SYSTEM_ID=A.SYSTEM_ID AND UPPER(ENT.PARENT_ENTITY_TYPE)=UPPER(A.PARENT_LAYER_NAME)
-- 	WHERE ENT.SYSTEM_ID='||P_SYSTEM_ID||'';
-- 	EXECUTE V_QUERY INTO V_MAPPED_PARENT;
-- 
-- 	IF V_MAPPED_PARENT IS NULL
-- 	THEN 
-- 		V_STATUS:=FALSE;
-- 		V_MESSAGE:=V_LAYER_TITLE||' should be inside the mapped '||V_LAYER_MAPPING.PARENT_LAYER_NAME||' only!';
-- 
-- 		RETURN QUERY
-- 		select V_STATUS as status, V_MESSAGE::character varying as message;
-- 	END IF;
...
IF (V_IS_ASSOCIATED) THEN
		...
		select geom_type into v_associate_entity_geom from layer_details where upper(layer_name)= upper(V_AROW.associated_entity_type);
		raise info 'v_associate_entity_geom%',v_associate_entity_geom;
		raise info 'v_associate_entity_geom%',V_AROW.associated_entity_type;

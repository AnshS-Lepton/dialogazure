...
Select Cast(st_length(sp_geometry,false)as decimal(10,2)) into v_cable_length from line_master where system_id =p_entity_system_id and entity_type= 'Cable';
IF(p_distance < v_cable_length) THEN
	
 SELECT ADB.CABLE_ID INTO p_fiber_cable_id FROM ATT_DETAILS_FIBER_LINK ATT
 INNER JOIN ATT_DETAILS_CABLE_INFO ADB ON ADB.LINK_SYSTEM_ID = ATT.SYSTEM_ID WHERE LINK_ID = p_fiber_link_id 
 AND fiber_number = p_entity_port_no AND ADB.CABLE_ID = p_entity_system_id LIMIT 1;
...
END IF;
	...
	RAISE INFO '%v_main_path', v_main_path;
		...
		THEN
	       ...
	       'z_end_entity_type', CB.b_entity_type , 'z_end_entity_id',CB.b_network_id ,'a_end_distance',Cast(st_distance(v_sub_line_end_point,v_sub_line_start_point,false)as decimal(10,2)),'z_end_distance',Cast(st_distance(st_endpoint(v_main_path) ,v_sub_line_end_point,false)as decimal(10,2)))
            ...
            INNER JOIN ATT_DETAILS_FIBER_LINK ADFL ON ADCI.LINK_SYSTEM_ID = ADFL.SYSTEM_ID AND fiber_number = p_entity_port_no  
 ...
 return query select row_to_json(x)
	            from (select 'Failed' AS status, 'Cable not found again Fiber link Id and port no.' AS error_message)x;
END IF;	
ELSE

 return query select row_to_json(x)
	            from (select 'Failed' AS status, 'optical_distance cannot be greater than cable length!' AS error_message)x;
END IF;	
...
ALTER FUNCTION public.fn_api_get_fault_location_detail(character varying, character varying, character varying, integer, double precision)
    OWNER TO postgres;

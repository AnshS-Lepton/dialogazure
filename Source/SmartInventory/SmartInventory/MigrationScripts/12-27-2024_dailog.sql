update res_resources set value='Site Name' where key='SI_OSP_GBL_NET_GBL_139' and culture='en'
------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
BEGIN
	...
	RAISE NOTICE 'This is a notice message11';
		...
		and upper(column_name)=upper(label_column))then
			if(lower(label_column)='(siteid-sitename)')then
...
VALUES((select layer_id from layer_details where layer_name='POD'), 'Info', '(siteid-sitename)', 12, '(siteid-sitename)', 'vw_att_details_pod_report', true, false, 1, now(), NULL, NULL, false, true, NULL);
---------------------------------------------------------------------------------------------------------------------------------------------------------------
update res_resources set is_jquery_used=true where key='SI_OSP_ROW_NET_GBL_021';
---------------------------------------------------------------------------------------------------------------------------------------------------------------
alter table att_details_cable
 add column parent_cable_system_id character varying(200);
 
 alter table att_details_cable
 add column parent_cable_netwok_id character varying(200);

alter table att_details_cable
 add column splited_by character varying(200);
 
alter table att_details_cable
 add column splitted_on timestamp without time zone DEFAULT now(),
 
 alter table att_details_cable
 add column splitting_system_id character varying(200);
 
 alter table att_details_cable
 add column splitting_netwok_id character varying(200);

alter table att_details_cable
 add column splitting_entityType character varying(200);
...
CREATE OR REPLACE VIEW public.vw_att_details_cable_report
 ...
 SELECT cable.system_id,
	...
	(select user_name AS splited_by from user_master where user_id = cable.splited_by::integer),

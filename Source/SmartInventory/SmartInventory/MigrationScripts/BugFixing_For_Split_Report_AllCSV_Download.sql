-- FUNCTION: public.fn_get_split_report_summary_view_allcsv(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, double precision, character varying)

-- DROP FUNCTION IF EXISTS public.fn_get_split_report_summary_view_allcsv(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, double precision, character varying);

CREATE OR REPLACE FUNCTION public.fn_get_split_report_summary_view_allcsv(
...
s_table_name := 'att_details_' || lower(p_layer_name);
...
sql:='SELECT STRING_AGG(( '''||v_column_name ||'''  )|| '''||v_csv_delimeter||''','''') 
 union all ';
 
IF ((p_geom) != '') THEN
--FETCH RECORD BASED ON SELECTED GEOMETRY.
sql:= sql || 'SELECT 
    REPLACE(COALESCE(parent_'||lower(p_layer_name)||'_netwok_id::text, ''''), E''\n'', '''') || ''|'' || 
    REPLACE(COALESCE(network_id::text, ''''), E''\n'', '''') || ''|'' || 
    REPLACE(COALESCE(splitting_netwok_id::text, ''''), E''\n'', '''') || ''|'' || 
    REPLACE(COALESCE(splitting_entityType::text, ''''), E''\n'', '''') || ''|'' || 
    REPLACE(COALESCE(split_by::text, ''''), E''\n'', '''') || ''|'' || 
    REPLACE(COALESCE(split_on::text, ''''), E''\n'', '''') 
from (select parent_'||lower(p_layer_name)||'_netwok_id,network_id,
splitting_netwok_id,splitting_entityType,
(SELECT user_master.user_name AS qaaa FROM user_master WHERE user_master.user_id = splited_by::integer) AS split_by,
TO_CHAR(splitted_on, ''DD-Mon-YY'') AS split_on from '||s_table_name||' a
...
sql:= sql || 'SELECT 
    ...
    REPLACE(COALESCE(split_by::text, ''''), E''\n'', '''') || ''|'' || 
    REPLACE(COALESCE(split_on::text, ''''), E''\n'', '''') 
from (select parent_'||lower(p_layer_name)||'_netwok_id, network_id, splitting_netwok_id, splitting_entityType, 
(SELECT user_master.user_name AS qaaa FROM user_master WHERE user_master.user_id = splited_by::integer ) AS split_by,
TO_CHAR(splitted_on, ''DD-Mon-YY'') AS split_on from '||s_table_name||' a';
...
sql:= sql ||')b where split_by is not null'; 
sql:= sql ||'';
--sql := sql || ' ORDER BY splited_by DESC'''; 

RAISE INFO 'QUERYtest %', sql;
RETURN QUERY
EXECUTE 'select row_to_json(row) from ('||sql||') row';
End;
$BODY$;

ALTER FUNCTION public.fn_get_split_report_summary_view_allcsv(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, double precision, character varying)
    OWNER TO postgres;
-----------------------------------------------------------------------------------
...
where   pe.role_id = p_roleid and pe.viewonly = true and L.isvisible = true and L.is_report_enable = true and L.layer_name in('Cable','Duct','Trench') 
...
sql:= sql ||' AND  0 = 1';
 ...
 sql:=sql||'AND a.splited_by is not null';
...
ALTER FUNCTION public.fn_get_split_report_summary(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, integer, integer, boolean, character varying, character varying, double precision, character varying)
    OWNER TO postgres;
-----------------------------------------------------------------------------------------------------------------------------------------
-- FUNCTION: public.fn_get_split_report_summary_view_allexcel(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, double precision, character varying)

-- DROP FUNCTION IF EXISTS public.fn_get_split_report_summary_view_allexcel(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, double precision, character varying);

CREATE OR REPLACE FUNCTION public.fn_get_split_report_summary_view_allexcel(
	p_regionids character varying,
	p_provinceids character varying,
	p_networkstatues character varying,
	p_parentusers character varying,
...
IF ((p_geom) != '') THEN
--FETCH RECORD BASED ON SELECTED GEOMETRY.
sql:= 'SELECT parent_'||lower(p_layer_name)||'_netwok_id, network_id As child_'||lower(p_layer_name)||'_netwok_id, splitting_netwok_id, splitting_entityType as splitting_entity_Type,
(SELECT user_master.user_name AS qaaa FROM user_master WHERE user_master.user_id = splited_by::integer) AS split_by,
TO_CHAR(splitted_on, ''DD-Mon-YY'') AS split_on
from '||s_table_name||' a
inner join temp_entity_inside_geom m on a.system_id=m.system_id
where 1=1';
ELSE
...
sql:= sql ||'where splited_by is not null order by splited_by desc' ;
...
ALTER FUNCTION public.fn_get_split_report_summary_view_allexcel(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, double precision, character varying)
    OWNER TO postgres;
------------------------------------------------------------------------------------------------------------------------------------------------
-- FUNCTION: public.fn_get_split_report_summary_view_allshape(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, double precision, character varying)

-- DROP FUNCTION IF EXISTS public.fn_get_split_report_summary_view_allshape(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, double precision, character varying);

CREATE OR REPLACE FUNCTION public.fn_get_split_report_summary_view_allshape(
	p_regionids character varying,
	p_provinceids character varying,
	p_networkstatues character varying,
	p_parentusers character varying,
...
IF ((p_geom) != '') THEN
--FETCH RECORD BASED ON SELECTED GEOMETRY.
sql:= 'SELECT '||v_column_name||'
from (select parent_'||lower(p_layer_name)||'_netwok_id, network_id as child_'||lower(p_layer_name)||'_netwok_id, splitting_netwok_id, splitting_entityType as splitting_entity_type,
(SELECT user_master.user_name AS qaaa FROM user_master WHERE user_master.user_id = splited_by::integer) AS split_by,
TO_CHAR(splitted_on, ''DD-Mon-YY'') AS split_on from '||s_table_name||' a
inner join temp_entity_inside_geom m on a.system_id=m.system_id
where 1=1';
...
sql:= sql ||' AND upper(a.network_status:: TEXT) in('||p_networkStatues||')';
END IF;
-- select * from fn_get_split_report_summary_view_allshape('','','P','0,5','5','Cable','','','','','Created_On','','','',0,0,'','','',5,2,'Own','','',0.0,'');
...
--sql:= sql ||' )X';
sql:= sql ||' where splited_by is not null)X';
...
ALTER FUNCTION public.fn_get_split_report_summary_view_allshape(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, double precision, character varying)
    OWNER TO postgres;

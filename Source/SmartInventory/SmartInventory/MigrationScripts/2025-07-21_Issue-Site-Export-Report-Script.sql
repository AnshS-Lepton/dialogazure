...
BEGIN
  ...
  CREATE TEMP TABLE temp_cable_route(
     ...
     select Value:: integer into v_buffer_temp from  global_settings where key='SiteBuffer';
     v_buffer:= v_buffer_temp;
   ...
   if (v_nearest_cable_end_geom is not null) then
    ...
    INSERT INTO temp_att_details_site  (system_id,network_id,site_geometry,start_point,nearest_cable_system_id,nearest_cable_end_geom,end_point)  
    SELECT 
        pm.system_id, 
    ...
    LIMIT 5;
           ...
           RAISE INFO 'Value of variable v_nearest_cable_system_id: %', v_nearest_cable_system_id;
    ...
    loop
     RAISE INFO 'Value of variable start_point: %', rec.start_point;
      ...
      delete from temp_cable_route where system_id = -1 
     and site_system_id = rec.system_id;
     
      if exists (select 1 from temp_cable_route where site_system_id =  rec.system_id)
      then 
          RAISE INFO 'Value of variable rec.system_id: %', rec.system_id;

     UPDATE temp_att_details_site tas
	SET 
	    nearest_cable_to_site_geometry = sub.merged_geom,
	    distance = sub.total_length
	FROM (
	    SELECT 
	        tc.site_system_id AS system_id,
	        ST_LineMerge(ST_Union(lm.sp_geometry)) AS merged_geom,
	        ST_Length(ST_LineMerge(ST_Union(lm.sp_geometry))::geography) AS total_length
	    FROM temp_cable_route tc
	    JOIN line_master lm ON lm.system_id = tc.system_id
	    where lm.network_status = 'A'
	    GROUP BY tc.site_system_id
	) sub
	WHERE tas.system_id = sub.system_id;
    ...
    ELSE     
        ...
        FROM (select site.nearest_cable_end_geom,site.nearest_cable_system_id,
 ...
 join temp_cable_route tmp on tmp.site_system_id = site.system_id) t order by t.distance limit 1;
    END IF;
 END IF;
END;
$function$

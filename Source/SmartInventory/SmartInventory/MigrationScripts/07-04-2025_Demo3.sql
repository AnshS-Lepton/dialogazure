...
alter table att_details_pod add no_of_sites integer;
alter table att_details_pod add max_distance_peer integer;
...
VALUES('Delete Site', 'Delete Site', '', '', NULL, now(), 287, now(), 'Web', true, 'TPLD', (select id from module_master where module_name='Topology Details' and module_abbr='TPL' ), 0, false);
   

   ------------------------------------------Update data -----------------------------------------------

   update layer_action_mapping set is_active=false  where layer_id=(select layer_id from layer_details where layer_name='POD') and action_name='TopologyPlan';
...
elsIF(upper(p_entity_type)='POD' and (select count(1) from user_module_mapping umm  where user_id=p_user_id and module_id=(select id from module_master where module_name='Delete Site' and module_abbr='TPLD'))=0)
then 
V_IS_VALID=FALSE;
...
select adp.site_id,adp.site_name,adp.top_type,tr.ring_capacity,ts.segment_code,ts.agg1_site_id ,ts.agg2_site_id,tr2.region_name,tr.ring_code,adp.ring_id,
COALESCE(adp.no_of_sites, 0) AS no_of_sites,COALESCE(adp.max_distance_peer, 0) AS max_distance_peer,adp.ring_a_site_id,adp.ring_b_site_id 
            ...
            inner join att_details_pod adp on adp.network_id=sm.site_id 
...
----------------------------------------Modify this function for ring details----------------------------

CREATE OR REPLACE FUNCTION public.fn_get_ring_assocication_detailstest(
...
BEGIN
    ...
    SELECT network_id INTO p_network_id 
    FROM att_details_cable 
    WHERE system_id = cable_id::INT;
    ...
    sql_query := 'SELECT row_to_json(row) FROM ( 
                  SELECT  
				  tr.ring_code,     
   				 tre.region_name AS region_name,  
  				  ts.segment_code, 
   				 tr.ring_capacity, 
  				  tr.id AS ring_id  
			FROM top_ring tr
			...
			inner join top_ring_cable_mapping tcm on tcm.ring_id=tr.id 
			where tcm.cable_id = $1';
...
ALTER FUNCTION public.fn_get_ring_assocication_detailstest(boolean, character varying, character varying, character varying, integer, character varying)
    OWNER TO postgres;
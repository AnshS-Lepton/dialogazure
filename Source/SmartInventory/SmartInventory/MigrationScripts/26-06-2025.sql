...
------------------------------------------ Insert data3 to search by sub link id---------------------------
...
VALUES('sub_link_id', 43, 'Sub Link Id', true, 1, now(), NULL, NULL, 'SI_OSP_GBL_NET_GBL_192', false);



-----------------------------------Get project details-----------------------------------

CREATE OR REPLACE FUNCTION public.get_site_project_detailsbyid(p_id integer)
 RETURNS SETOF json
 LANGUAGE plpgsql
AS $function$
BEGIN
...
;
-- Permissions
ALTER FUNCTION public.get_site_project_detailsbyid(int4) OWNER TO postgres;
GRANT ALL ON FUNCTION public.get_site_project_detailsbyid(int4) TO postgres;

---------------------------Modify function to get fn_get_fiber_link_status----------------------------
...
BEGIN
    ...
    and id in (2,3,29,40,43) order by column_sequence) ROW';
...
ALTER FUNCTION public.fn_get_fiber_link_columns_mappings()
    OWNER TO postgres;


    ---------------------------------------------------Gte fiber link details------------------------------
...
--  END IF;
 ...
 IF ((p_searchtext) != '' and (p_searchby) != '') THEN
 IF p_searchby = 'sub_link_id' THEN 
 select fb.main_link_id into main_link_Id from vw_att_details_fiber_link fb where fb.link_id=p_searchtext;
 -- Replace values
        p_searchby := 'link_id';        -- overwrite old value
        p_searchtext := main_link_Id;   -- assign new value from SELECT
 end if;
...
GRANT EXECUTE ON FUNCTION public.fn_get_fiber_link_details(integer, character varying, character varying, integer, integer, character varying, character varying, integer, timestamp without time zone, timestamp without time zone) TO PUBLIC;

GRANT EXECUTE ON FUNCTION public.fn_get_fiber_link_details(integer, character varying, character varying, integer, integer, character varying, character varying, integer, timestamp without time zone, timestamp without time zone) TO postgres;

-- DROP FUNCTION public.fn_save_entity_geom(int4, varchar, varchar, int4, varchar, varchar, varchar, varchar, varchar, varchar, float8, int4);

CREATE OR REPLACE FUNCTION public.fn_save_entity_geom(p_system_id integer, p_geom_type character varying, p_entity_type character varying, p_userid integer, p_longlat character varying, p_common_name character varying, p_network_status character varying, p_ports character varying, p_entity_category character varying, p_center_line_geom character varying, p_buffer_width double precision, p_project_id integer)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
...
select lower(layer_table) into v_layer_table from layer_details where Upper(layer_name) = upper(p_entity_type) ;

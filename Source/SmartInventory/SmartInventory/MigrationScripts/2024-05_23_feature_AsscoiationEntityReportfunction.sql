...
ALTER FUNCTION public.fn_get_association_report_summary_view(character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, integer, integer, character varying, character varying, character varying, double precision, character varying)
    OWNER TO postgres;
...
DROP VIEW public.vw_associate_entity_info;
CREATE OR REPLACE VIEW public.vw_associate_entity_info
 AS
 SELECT  a.entity_system_id AS system_id,
    a.entity_network_id,
     ld1.layer_title  as entity_type,
     a.entity_type as entity_layer_name,
    a.associated_system_id,
    a.associated_network_id,
    ld.layer_title  as associated_entity_type,
    a.associated_entity_type as associated_entity_layer_name, 
    a.created_by,
    fn_get_date(a.created_on, false) AS associated_on,
    b.user_name AS associated_by
   FROM associate_entity_info a 
     JOIN user_master b ON a.created_by = b.user_id
     inner join layer_details ld on ld.layer_name = a.associated_entity_type
     inner join layer_details ld1 on ld1.layer_name = a.entity_type;

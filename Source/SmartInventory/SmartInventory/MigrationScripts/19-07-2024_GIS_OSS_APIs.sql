...
-- select fn_api_get_intermediateentities('FDP','WAI-POD047-FMS01','ONT','GGN-ONT03',1)
...
DECLARE
	...
	v_destination_layer_table_name text;
	v_destination_layer_name character varying ;
...
BEGIN
	...
	SELECT layer_table, layer_name INTO v_destination_layer_table_name, v_destination_layer_name
    FROM layer_details
    WHERE layer_title = p_destination_entity_type; -- search by title and use layer name
	
	IF v_layer_table_name IS NULL OR v_layer_name IS NULL THEN
        RETURN QUERY SELECT json_build_object(
            'error', 'Details not found for source entity type ' || p_source_entity_type
        )::json;
        DROP TABLE IF EXISTS cpf_temp_result;
        RETURN;
    END IF;
	
	IF v_destination_layer_table_name IS NULL OR v_destination_layer_name IS NULL THEN
        RETURN QUERY SELECT json_build_object(
            'error', 'Details not found for destination entity type ' || p_destination_entity_type
        )::json;
        DROP TABLE IF EXISTS cpf_temp_result;
        RETURN;
    END IF;
   ...
   IF v_source_system_id IS NULL THEN
        RETURN QUERY SELECT json_build_object(
            'error', 'Details not found for source ID ' || p_source_id
        )::json;
        DROP TABLE IF EXISTS cpf_temp_result;
        RETURN;
    END IF;
	...
	v_sql_query1 := 'SELECT (row_to_json(t) ->> ''system_id'')::integer
                  FROM (
                      SELECT lt.system_id
                      FROM ' || quote_ident(v_destination_layer_table_name) || ' lt
                      JOIN point_master pm ON lt.network_id = pm.common_name
                      WHERE lt.network_id = ''' || p_destination_id || '''
                      LIMIT 1
                  ) t';
	
	EXECUTE v_sql_query1 INTO v_destination_system_id;
   
   IF v_destination_system_id IS NULL THEN
        RETURN QUERY SELECT json_build_object(
            'error', 'Details not found for destination ID ' || p_destination_id
        )::json;
        DROP TABLE IF EXISTS cpf_temp_result;
        RETURN;
    END IF;
...
IF v_layer_table_name IS NULL OR v_layer_name IS NULL THEN
        RETURN QUERY SELECT json_build_object(
            'error', 'Details not found for Entity type ' || p_entity_type
        )::json;

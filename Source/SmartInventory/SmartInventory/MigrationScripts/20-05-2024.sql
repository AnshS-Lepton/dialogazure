...
-- FUNCTION: public.fn_update_line_associated_child_geometry(integer, character varying, integer)

-- DROP FUNCTION IF EXISTS public.fn_update_line_associated_child_geometry(integer, character varying, integer);
	...
	LOOP 
		...
		then
			...
			LOOP
				...
				UPPER(ASSOCIATED_ENTITY_TYPE)!='CABLE';
				delete from connection_info where (upper(source_entity_type)=upper(V_BROW.associated_entity_type) and source_system_id=V_BROW.associated_system_id) or 
			(upper(destination_entity_type)=upper(V_BROW.associated_entity_type) and destination_system_id=V_BROW.associated_system_id);
				perform(fn_geojson_update_entity_attribute(V_BROW.associated_system_id,V_BROW.associated_entity_type,0,1,true));
			END LOOP;
...
ALTER FUNCTION public.fn_update_line_associated_child_geometry(integer, character varying, integer)
    OWNER TO postgres;

...
begin
	v_sql:='';
	v_RADIUS = 1000;
	--select  * from fn_get_vector_layers_delta_by_geom('2024-02-02 13:53:57.174886','156',-1.27602,36.82734,53);
	if coalesce(p_lastFetchTime, '') <> '' then
		...
		loop
			...
			ELSE 
				v_sql:=v_sql||'  
				WHERE EXISTS(SELECT 1 FROM '||v_arow.delta_metadata_table||' d WHERE d.province_id = e.province_id and 
				d.province_id in ('||p_prvinceids||')
				and d.last_delta_at >= '''||(p_lastFetchTime)||'''::timestamp) ';
			END IF;
		end loop;
		...
		IF  p_ticket_id = 0 Then			
			...
			(row.geom_type = ''Point'' AND exists(select 1 from point_master p where p.entity_type = row.layer and p.system_id = row.system_id AND
					 ST_INTERSECTS(P.SP_GEOMETRY, ST_BUFFER_METERS(ST_GEOMFROMTEXT(''POINT('|| p_longitude||' '||p_latitude||')'',4326),'||v_RADIUS||'))))
			OR
			(row.geom_type = ''Line'' AND exists(select 1 from line_master p where p.entity_type = row.layer and p.system_id = row.system_id AND
					 ST_INTERSECTS(P.SP_GEOMETRY, ST_BUFFER_METERS(ST_GEOMFROMTEXT(''POINT('|| p_longitude||' '||p_latitude||')'',4326),'||v_RADIUS||'))))
			OR
			((row.geom_type = ''Polygon'' OR row.layer = ''Building'')  AND exists(select 1 from Polygon_master p where p.entity_type = row.layer and p.system_id = row.system_id AND
					 ST_INTERSECTS(P.SP_GEOMETRY, ST_BUFFER_METERS(ST_GEOMFROMTEXT(''POINT('|| p_longitude||' '||p_latitude||')'',4326),'||v_RADIUS||'))))
			';
		Else
			RETURN QUERY EXECUTE 'select row_to_json(row) as geo from ('||v_sql||')row 

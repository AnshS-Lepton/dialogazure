...
AS $BODY$
--select * from fn_get_association_report_summary_view_csv('','','','1','','MicrowaveLink','','','','','Cable','','','',0,0,'','','',5,97,'','','',0,'')
...
S_LAYER_COLUMNS_CSV text;
s_layer_columns_val text;
v_column_name character varying;
v_temp_column_name character varying;
v_temp_layer_table character varying;
 rec record;
...
v_csv_delimeter:='^';
 ...
 v_column_name := CASE
 WHEN p_layer_name = 'Tower' OR p_layer_name = 'Sector' OR p_layer_name = 'Antenna' OR p_layer_name = 'Conduit' OR p_layer_name = 'MicrowaveLink' THEN 'network_name'
 WHEN p_layer_name = 'Fault' OR p_layer_name = 'Slack' OR p_layer_name = 'Loop' OR p_layer_name = 'Competitor' THEN 'network_id'
 ELSE lower(p_layer_name) || '_name'
...
ctesql:='with ctequery as(
 select ASS.system_id,ASS.entity_network_id, ASS.entity_type,ASS.associated_system_id,ASS.associated_network_id,ASS.associated_entity_type,
	ASS.associated_on,ASS.associated_by from vw_associate_entity_info ASS where 
       upper(ass.entity_layer_name)=upper('''||s_layer_name||''')
      union
	 select ASS.system_id,ASS.entity_network_id, ASS.entity_type,ASS.associated_system_id,ASS.associated_network_id,ASS.associated_entity_type,ASS.associated_on,ASS.associated_by
	from vw_associate_entity_info ASS where 
	 upper(ass.associated_entity_layer_name)=upper('''||s_layer_name||''')
)';
-- DYNAMIC QUERY
IF ((p_geom) != '') THEN
--FETCH RECORD BASED ON SELECTED GEOMETRY.
sql:= ctesql || '   SELECT ROW_NUMBER() OVER (ORDER BY '|| LowerStart || CASE WHEN (P_SORTCOLNAME) = '' THEN 'system_id' ELSE P_SORTCOLNAME END
...
ASS.associated_on,ASS.associated_by,a.'||v_column_name||' as associated_entity_name   from '||S_REPORT_VIEW_NAME||' a
...
sql:= sql ||'  inner join ctequery ASS on ASS.system_id = a.system_id';
...
raise info 'sql TotalRecords %',TotalRecords;

-- RAISE EXCEPTION 'Calling result(%)', sql;
--GET PAGE WISE RECORD (FOR PARTICULAR PAGE)
 drop table if exists associated_temp_entity_report_data_csv;
IF((P_PAGENO) <> 0) THEN
StartSNo:= P_PAGERECORD * (P_PAGENO - 1 ) + 1;
EndSNo:= P_PAGENO * P_PAGERECORD;

raise notice 'sql %',sql;
sql:= 'create temp table associated_temp_entity_report_data_csv as SELECT '||TotalRecords||' as totalRecords, *
FROM (' || sql || ' ) T
...
sql:= 'create temp table associated_temp_entity_report_data_csv as  SELECT '||TotalRecords||' as totalRecords, * FROM (' || sql || ') T';
END IF;
 raise info 'sql query for insertion %', sql;
 Execute sql;
     ...
     FOR rec IN
        SELECT layer_title, layer_name, layer_table
        FROM layer_details
        WHERE layer_title IN (SELECT DISTINCT associated_entity_type FROM associated_temp_entity_report_data_csv)
    LOOP
        -- Determine the appropriate column name based on the layer_name
        v_temp_column_name := '';
        v_temp_column_name := CASE
            WHEN rec.layer_name = 'Tower' OR rec.layer_name = 'Sector' OR rec.layer_name = 'Antenna' OR rec.layer_name = 'Conduit' OR rec.layer_name = 'MicrowaveLink' THEN 'network_name'
            WHEN rec.layer_name = 'Fault' OR rec.layer_name = 'Slack' OR rec.layer_name = 'Loop' OR rec.layer_name='Competitor' THEN 'network_id'
            ELSE lower(rec.layer_name) || '_name'
        END;

        -- Construct the dynamic SQL for the update
        EXECUTE format(
            'Update associated_temp_entity_report_data_csv T
             SET associated_entity_name = ent.'||v_temp_column_name||' 
             from '||rec.layer_table||' ent
             where  T.associated_entity_type = '''|| rec.layer_title ||''' and  t.associated_network_id = ent.network_id'     
        );
        
     end loop;
     
s_column_name:= 'entity_network_id^entity_type^entity_name^associated_network_id^associated_entity_type^associated_on^associated_by^Associated_entity_name';

sql_csv:='SELECT '||TotalRecords ||' as totalRecords , 0 as S_No,STRING_AGG(( '''||s_column_name ||'''  )|| '''||v_csv_delimeter||''','''') 
  union all  ';
sql_csv:= sql_csv || ' select totalrecords,S_No,
    ...
    REPLACE(COALESCE(associated_entity_name::text, ''''), E''\n'', '''') as STRING_AGG from  associated_temp_entity_report_data_csv order by S_No' ; 
    raise info 'print queryyyy final %', sql_csv;
 
RETURN QUERY EXECUTE 'select row_to_json(row) from ('||sql_csv||') row';
End;
$BODY$;
...
--select * from fn_get_association_report_summary_view('','','','1','','Cable','','','','','Created_On','','','',0,0,'','','',5,97,'','','',0,'')
...
-- RAISE EXCEPTION 'Calling result(%)', sql;
--GET PAGE WISE RECORD (FOR PARTICULAR PAGE)
 drop table if exists associated_temp_entity_report_data_excel;
IF((P_PAGENO) <> 0) THEN
StartSNo:= P_PAGERECORD * (P_PAGENO - 1 ) + 1;
EndSNo:= P_PAGENO * P_PAGERECORD;

raise notice 'sql %',sql;
sql:= 'create temp table associated_temp_entity_report_data_excel as SELECT '||TotalRecords||' as totalRecords, *
FROM (' || sql || ' ) T
WHERE S_No BETWEEN ' || StartSNo || ' AND ' || EndSNo;
...
sql:= 'create temp table associated_temp_entity_report_data_excel as  SELECT '||TotalRecords||' as totalRecords, * FROM (' || sql || ') T';
END IF;
 raise info 'sql query for insertion %', sql;
 Execute sql;
     ...
     FOR rec IN
        SELECT layer_title, layer_name, layer_table
        FROM layer_details
        WHERE layer_title IN (SELECT DISTINCT associated_entity_type FROM associated_temp_entity_report_data_excel)
    LOOP
        -- Determine the appropriate column name based on the layer_name
        v_temp_column_name := '';
        v_temp_column_name := CASE
        ...
        -- Construct the dynamic SQL for the update
        EXECUTE format(
            'Update associated_temp_entity_report_data_excel T
             SET associated_entity_name = ent.'||v_temp_column_name||' 
             from '||rec.layer_table||' ent
             where  T.associated_entity_type = '''|| rec.layer_title ||''' and  t.associated_network_id = ent.network_id'     
        );
   ...
   s_query:='Select * from associated_temp_entity_report_data_excel' ;          
RETURN QUERY EXECUTE 'select row_to_json(row) from ('||s_query||') row';
End;
$BODY$;

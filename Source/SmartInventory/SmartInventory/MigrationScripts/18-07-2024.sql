alter table att_details_fms add installation_location_code character varying;
alter table vendor_master add abbr character varying(20);

truncate table codification_layer_details restart identity;
...
Select layer_id,'Manhole_types','LC',true,1,now(),'LC','Manhole_types',true,true,0 from layer_details where upper(layer_name)='MANHOLE'
union
Select layer_id,'Manhole_types','JC',true,1,now(),'JC','Manhole_types',true,true,0 from layer_details where upper(layer_name)='MANHOLE'
union
Select layer_id,'Manhole_types','TC1',true,1,now(),'TC1','Manhole_types',true,true,0 from layer_details where upper(layer_name)='MANHOLE'
union
Select layer_id,'Manhole_types','TC2',true,1,now(),'TC2','Manhole_types',true,true,0 from layer_details where upper(layer_name)='MANHOLE'; 
...
update vendor_master set abbr=substring(upper(name),0,3);

update dropdown_master set dropdown_value='TK' where layer_id=(select layer_id from layer_details where layer_name='Cable')  and dropdown_type='Cable_Category' and dropdown_key='Trunk';
update dropdown_master set dropdown_value='BB' where layer_id=(select layer_id from layer_details where layer_name='Cable')  and dropdown_type='Cable_Category' and dropdown_key='Backbone';
update dropdown_master set dropdown_value='AC' where layer_id=(select layer_id from layer_details where layer_name='Cable')  and dropdown_type='Cable_Category' and dropdown_key='Access';
update dropdown_master set dropdown_value='LM' where layer_id=(select layer_id from layer_details where layer_name='Cable')  and dropdown_type='Cable_Category' and dropdown_key='LMC';
update dropdown_master set dropdown_value='AR' where layer_id=(select layer_id from layer_details where layer_name='Cable')  and dropdown_type='Cable_Category' and dropdown_key='Aerial';
...
THEN
	...
	THEN
		...
		THEN
			SELECT MANHOLE_TYPES INTO V_B_END_CODE FROM ATT_DETAILS_MANHOLE WHERE SYSTEM_ID=V_A_END_SYSTEM_ID;
		ELSIF (UPPER(V_B_END_ENTITY_TYPE)='MANHOLE')
		THEN
			SELECT MANHOLE_TYPES INTO V_B_END_CODE FROM ATT_DETAILS_MANHOLE WHERE SYSTEM_ID=V_B_END_SYSTEM_ID;
		END IF;
	END IF;

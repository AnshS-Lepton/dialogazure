...
----------------------------------------- Get stucture details---------------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_get_connected_structure(
	lat double precision,
	lng double precision,
	p_network_id character varying)
    RETURNS TABLE(cable_network_id character varying, cable_geom geometry, structure_id character varying, structure_type character varying, structure_geom geometry, longitude double precision, latitude double precision, distance_meters double precision) 
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE PARALLEL UNSAFE
    ROWS 1
...
BEGIN
    ...
    -- STEP 1: Get path point geometry using  network_id
    ...
    -- STEP 2: Find NEAREST structure (Pole/Manhole) intersecting 2m buffer of cable
    RETURN QUERY
    SELECT 
        v_network_id AS cable_network_id,
        v_cable_geom AS cable_geom,
        pm.COMMON_NAME AS structure_id,
        pm.entity_type AS structure_type,
        pm.sp_geometry AS structure_geom,

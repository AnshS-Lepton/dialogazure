...
CREATE OR REPLACE FUNCTION public.fn_validate_geom_update(p_system_id integer, p_geom_type character varying, 
p_entity_type character varying, p_userid integer, p_longlat character varying, p_bld_buffer integer,
p_source_ref_type character varying, p_source_ref_id character varying)
...
if COALESCE(p_source_ref_type,'')!='' and  COALESCE(p_source_ref_id,'')!='' then
...
IF (V_IS_ASSOCIATED) THEN
		...
		IF(v_is_shifting_allowed = false OR v_is_snapping_allowed =false) THEN
			RETURN QUERY SELECT FALSE  AS STATUS, (P_ENTITY_TYPE||'[SI_GBL_GBL_GBL_GBL_049]')::CHARACTER VARYING AS MESSAGE;
...
THEN	
	...
	then
		RETURN QUERY SELECT FALSE  AS STATUS, (P_ENTITY_TYPE||'[SI_GBL_GBL_GBL_GBL_019]')::CHARACTER VARYING AS MESSAGE;--start point can not be changed, as it is spliced with some entity!
	...
	then
		RETURN QUERY SELECT FALSE  AS STATUS, (P_ENTITY_TYPE||'[SI_OSP_GBL_GBL_GBL_146]')::CHARACTER VARYING AS MESSAGE;--end point can not be changed, as it is spliced with some entity!
	end if;

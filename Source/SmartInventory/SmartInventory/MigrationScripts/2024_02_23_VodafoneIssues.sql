alter table att_details_cable add column route_name character varying;
alter table audit_att_details_cable add column route_name character varying;

update layer_action_mapping set is_web_action=false where layer_id=(select layer_id from layer_details where layer_name='Building') and action_name='Print';
update layer_details set is_report_enable=false where layer_name in('ROW','FAULT','CABINET','LOOP','RACK','EQUIPMENT');
...
union
select slack.system_id,slack.network_id, 'NA' as associated_entity_type, 'NA' as associated_network_id,slack.slack_length,slack.system_id as slack_system_id, 
slack.created_by,slack.created_on,slack.modified_by, slack.modified_on from att_details_slack slack where 
slack.system_id=p_duct_system_id and  associated_network_id is null and associated_entity_type is null
) row;
...
CREATE OR REPLACE FUNCTION public.fn_delete_split_entity(p_system_id integer, p_entity_type character varying)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$
DECLARE
...
v_result:=false;	
	...
	EXECUTE 'select layer_table from layer_details where upper(layer_name)=upper('''||p_entity_type||''')' into v_layer_table;
	
  	EXECUTE 'delete from '||v_layer_table||' where system_id='||p_system_id||'';
	delete from associate_entity_info where (associated_system_id=p_system_id and upper(associated_entity_type)=upper(p_entity_type)) or
	 (entity_system_id=p_system_id and upper(entity_type)=upper(p_entity_type)) and is_termination_point=true;
	delete from associate_entity_info where(entity_system_id=p_system_id and upper(entity_type)= upper(p_entity_type)) and upper(associated_entity_type)
	 in  (select upper(associate_layer_name) from vw_associate_entity_master where upper(layer_name)=upper(p_entity_type)) and is_termination_point=false;
	delete from connection_info  where (source_system_id=p_system_id and upper(source_entity_type)=upper(p_entity_type)) or (destination_system_id=p_system_id and upper(destination_entity_type)=upper(p_entity_type));
	...
	delete from att_details_cable_info where cable_id=p_system_id;
	END IF;
...
END;$function$
;
CREATE OR REPLACE FUNCTION public.fn_grouplibrary_save_point_entity(p_id integer, p_gtype character varying)
...
CREATE OR REPLACE FUNCTION public.fn_save_child_group_library(p_entity_type character varying, p_network_id character varying, p_parent_system_id integer, p_parent_entity_type character varying, p_longitude double precision, p_latitude double precision, p_created_by integer, p_id integer)
 RETURNS TABLE(status boolean, message character varying)
 LANGUAGE plpgsql
AS $function$
DECLARE
...
then (V_INPUT_PORT||':'||V_OUTPUT_PORT)::text
when(coalesce(V_INPUT_PORT,0)>0 and coalesce(V_OUTPUT_PORT,0)>0) then V_INPUT_PORT::text else '' end));
...
CREATE OR REPLACE FUNCTION public.fn_grouplibrary_save_entity(p_properties character varying)
...
update layer_action_mapping set is_visible=false where layer_id=(select layer_id from layer_details where layer_name='Handhole') and action_name='Cancel';
...
INSERT INTO public.layer_mapping (layer_id, parent_layer_id, parent_sequence, is_enable_inside_parent_info, is_used_for_network_id, network_code_format, is_default_code_format,  is_default_parent)
 VALUES((select layer_id from layer_details where layer_title='FAT'), (select layer_id from layer_details where layer_title='Handhole'), 4, false, true, 'FATnnnn', true, false);
INSERT INTO public.layer_mapping (layer_id, parent_layer_id, parent_sequence, is_enable_inside_parent_info, is_used_for_network_id, network_code_format, is_default_code_format,  is_default_parent)
 VALUES((select layer_id from layer_details where layer_title='FDC'), (select layer_id from layer_details where layer_title='Handhole'), 4, false, true, 'FDCxxxx', true, false);

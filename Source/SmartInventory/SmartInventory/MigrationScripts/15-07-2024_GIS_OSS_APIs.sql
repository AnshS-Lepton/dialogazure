   ...
   SELECT layer_table, layer_name INTO v_layer_table_name, v_layer_name
    ...
    v_sql_query := 'SELECT row_to_json(row) 
                  FROM (
                      ...
                      INNER JOIN province_boundary pb ON lt.province_id = pb.id
                      INNER JOIN region_boundary rb ON lt.region_id = rb.id
...
DECLARE
    ...
    p_port integer := 1; 
    p_source_entity_type character varying := 'Cable'; 
	v_total_distance_meters double precision;
	v_sql_query text;
	v_layer_table_name text;
	v_layer_name character varying := 'Cable';
...
BEGIN
   ...
   EXECUTE v_sql_query INTO v_source_system_id;
	...
	from fn_get_connection_info('', '', 0, 0, '', '', 277464, p_port, v_layer_name)a
    ...
    SELECT json_build_object(
        ...
        'SourceEntity', json_build_object(
            'entity_type', v_layer_name,
            'entity_id', p_source_id
        ),
		'DestinationEntity', json_build_object(
...
RETURNS TABLE(error_message text, status text, results text)
LANGUAGE 'plpgsql'
COST 100
VOLATILE PARALLEL UNSAFE
AS $BODY$
...
BEGIN
   ...
   EXECUTE v_sql_query INTO v_source_system_id;
    ...
    SELECT null AS error_message,
           'true' AS status, 
           CONCAT(v_layer_name, ' Update successful')::text AS results;

EXCEPTION
     WHEN OTHERS THEN
        -- Return error message in case of an exception
        RETURN QUERY
        SELECT CONCAT(v_layer_name, ': ', SQLERRM)::text AS error_message,
               'false' AS status,
               null AS results;
...
ALTER FUNCTION public.fn_api_Update_AlarmStatusetails(character varying, character varying, INT, character varying, character varying)
    OWNER TO postgres;
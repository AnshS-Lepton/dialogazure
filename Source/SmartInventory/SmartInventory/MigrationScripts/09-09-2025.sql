...
------------------------------------------- 
CREATE OR REPLACE FUNCTION public.fn_get_linestring(
	p_latitude1 double precision,
	p_longitude1 double precision,
	p_latitude2 double precision,
	p_longitude2 double precision)
    RETURNS TABLE(geom text, length_meters double precision) 
    LANGUAGE 'plpgsql'
    COST 100
...
AS $BODY$
BEGIN
    geom := st_astext( ST_SetSRID(
                ST_MakeLine(
                    ST_MakePoint(p_longitude1, p_latitude1),
                    ST_MakePoint(p_longitude2, p_latitude2)
                ), 4326
            ));
...
---------------------------------------------get nearest fiber path point details------------------------------

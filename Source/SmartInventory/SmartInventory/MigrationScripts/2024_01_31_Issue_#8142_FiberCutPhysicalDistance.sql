...
raise info 'v_sub_line_geom: %',st_astext(v_sub_line_geom);
raise info 'v_faulty_record.cable_id:%',v_faulty_record.cable_id;
...
IF(coalesce(v_a_data_exist,'')='' OR coalesce(v_b_data_exist,'')='' and (v_a_data_exist is not null or v_b_data_exist is not null )   ) THEN

...
AS $BODY$
--select * from fn_get_association_report_summary_view('','','','1','','Pole','','','','','Created_On','','','',0,0,'','','',5,97,'','','',0,'')
...
from (select Ass.system_id,ASS.entity_network_id,a.'||p_layer_name||'_name as entity_name,ASS.entity_type,ASS.associated_network_id,ASS.associated_entity_type,
ASS.associated_on,ASS.associated_by,adt.'||p_layer_name||'_name as associated_entity_name from '||S_REPORT_VIEW_NAME||' a
...
from (select Ass.system_id,ASS.entity_network_id,ASS.entity_type,a.'||p_layer_name||'_name as entity_name,ASS.associated_network_id,ASS.associated_entity_type,
ASS.associated_on,ASS.associated_by,adt.'||p_layer_name||'_name as associated_entity_name from '||S_REPORT_VIEW_NAME||' a ';
...
inner join att_details_'||p_layer_name||' adt on adt.system_id = ASS.associated_system_id';
...
from (select ASS.system_id,ASS.entity_network_id,a.'||p_layer_name||'_name as as name,ASS.entity_type,
ASS.associated_network_id,ASS.associated_entity_type,ASS.associated_on,ASS.associated_by,adt.'||p_layer_name||'_name as associated_entity_name from '||S_REPORT_VIEW_NAME||' a
inner join temp_entity_inside_geom m on a.system_id=m.system_id
and upper(m.entity_type)=upper('''||s_layer_name||''') where 1=1 ';
...
from (select ASS.system_id,ASS.entity_network_id,a.'||p_layer_name||'_name as entity_name, ASS.entity_type,
ASS.associated_network_id,ASS.associated_entity_type,ASS.associated_on,ASS.associated_by,adt.'||p_layer_name||'_name as associated_entity_name from '||S_REPORT_VIEW_NAME||' a ';

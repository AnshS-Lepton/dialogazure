...
ALTER TABLE public.user_fe_tool_mapping ADD image_value boolean default false;
ALTER TABLE public.user_fe_tool_mapping ADD document_value boolean default false;
-----------------------------------------------------------------------------------------------------------------------------------------

CREATE TABLE public.fe_tools_attachement (
	...
	fe_tool_id int8 NULL,
	fe_userid int4 DEFAULT 0 NULL,
	tools_id int4 DEFAULT 0 NULL
);
	...
	if (searchby !~* '^[a-zA-Z0-9\s_]*$' or searchtext !~* '^[a-zA-Z0-9\s_]*$' or p_sortcolname !~* '^[a-zA-Z0-9\s_]*$' or
		...
		BEGIN
			...
			(P_SORTTYPE) ='' THEN 'desc' else P_SORTTYPE end||') AS S_No,id,user_name, tool_name,barcode, serial_number,  
			 ...
			 created_by_text, modified_by_text,date_value,image_value,document_value FROM vw_fe_tools_master WHERE 1=1';

update module_master set module_description='FMS Splicing Report' where module_name='FiberAllocationReport';

update layer_action_mapping set action_title='FMS Splicing Report' where action_name='FiberAllocationReport';
----------------------------------------------------------------------------------------------------------------------------------
...
ALTER FUNCTION public.fn_get_fms_connection(integer, character varying)
    OWNER TO postgres;
-----------------------------------------------------------------------------------------------------------------
-- FUNCTION: public.fn_get_entity_export_log(character varying, character varying, integer, integer, character varying, character varying, integer, character varying, character varying)
	...
	-- COLUMN SEQUENCE CHANGED AS SEARCH ENTITY ALWAYS SHOULD IN LEFT SIDE..
	   ...
	   , 'A' as trace_end from (
		with RECURSIVE UpStream as (
			...
			and (case when ci.destination_entity_type='FMS' and ci.destination_port_no<0 then 1=2 else 1=1 end)
	 ...
	 select b.*,'B' as trace_end from (
		with RECURSIVE DownStream as (
			...
			and (case when ci.source_entity_type='FMS' and ci.source_port_no<0 then 1=2 else 1=1 end) 			
			and cte.rowid<=v_max_row_count 

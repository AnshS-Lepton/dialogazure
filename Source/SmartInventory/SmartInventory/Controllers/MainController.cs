...
objEntityLstCount.objFilterAttributes.selection_type, objEntityLstCount.objFilterAttributes.buff_Radius, objEntityLstCount.objFilterAttributes.dd_networkStatus);
        ...
        [HttpGet]
        public JsonResult GetNearByEntitiesByLatLong(double latitude, double longitude, int bufferInMtrs)
        {
            try
            {
                ...
                var filteredResults = response.Where(r => r.entity_type == EntityType.ODF.ToString() || r.entity_type == EntityType.SpliceClosure.ToString()).ToList();

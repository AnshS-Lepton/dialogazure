...
{
    public class BLTempMicroduct : BLDataUploader
    {
        DATempMicroduct datempMicroduct;
        public BLTempMicroduct()
        {
            datempMicroduct = new DATempMicroduct();
            datempMicroduct.DataUploaderNotificationHandler += NotifyUpdatedStatus;
        }
        public void Save(List<TempMicroduct> lstMicroduct)
        {
            datempMicroduct.Save(lstMicroduct);
        }
        public List<TempMicroduct> GetAll(int UploadId)
        {
            return datempMicroduct.GetAll(UploadId);
        }
        public List<ErrorMessage> InsertMicroductIntoMainTable(UploadSummary summary)
        {
            //datempMicroduct.InsertMicroductIntoMainTable(summary);
        ...
        private List<ErrorMessage> ParallelInsert(UploadSummary summary)
        {
            ...
            List<TempMicroduct> lstTempADB = GetAll(summary.id);
            ...
            ParallelLoopResult result = Parallel.ForEach(lstBatch, options, (batchId) =>
            {
                try
                {
                    lock (lockObj)
                    {
                        datempMicroduct.InsertMicroductIntoMainTable(summary, batchId);

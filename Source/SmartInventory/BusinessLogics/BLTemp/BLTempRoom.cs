...
{
  public  class BLTempRoom : BLDataUploader
    {
        DATempRoom daTemplate;
        public BLTempRoom()
        {
            daTemplate = new DATempRoom();
            daTemplate.DataUploaderNotificationHandler += NotifyUpdatedStatus;
        }
        public void Save(List<TempRoom> lstRoom)
        {
            daTemplate.Save(lstRoom);
        }

        public List<ErrorMessage> InsertRoomIntoMainTable(UploadSummary summary)
        {
        ...
        private List<ErrorMessage> ParallelInsert(UploadSummary summary)
        {
            ...
            List<TempRoom> lstTemp = GetAll(summary.id);
            ...
            ParallelLoopResult result = Parallel.ForEach(lstBatch, options, (batchId) =>
            {
                try
                {
                    lock (lockObj)
                    {
                        daTemplate.InsertRoomIntoMainTable(summary, batchId);

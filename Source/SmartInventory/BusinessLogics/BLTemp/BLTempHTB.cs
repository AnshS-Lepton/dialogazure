...
{
    public class BLTempHTB : BLDataUploader
    {
        DATempHTB daTempObject;

        public BLTempHTB()
        {
            daTempObject = new DATempHTB();
            daTempObject.DataUploaderNotificationHandler += NotifyUpdatedStatus;
        }
        public void Save(List<TempHTB> lstHTB)
        {
            daTempObject.Save(lstHTB);
        }

        public List<ErrorMessage> InsertHTBIntoMainTable(UploadSummary summary)
        {
        ...
        private List<ErrorMessage> ParallelInsert(UploadSummary summary)
        {
            ...
            List<TempHTB> lstTemp = GetAll(summary.id);
            ...
            ParallelLoopResult result = Parallel.ForEach(lstBatch, options, (batchId) =>
            {
                try
                {
                    lock (lockObj)
                    {
                        daTempObject.InsertHTBIntoMainTable(summary, batchId);

...
{
    public class BLTempSpliceClosure : BLDataUploader
    {
        DATempSpliceClosure dATempSpliceClosure;
        public BLTempSpliceClosure()
        {
            dATempSpliceClosure = new DATempSpliceClosure();
            dATempSpliceClosure.DataUploaderNotificationHandler += NotifyUpdatedStatus;

        }
        public void Save(List<TempSpliceClosure> lstSpliceClosure)
        {
            dATempSpliceClosure.Save(lstSpliceClosure);
        }
        public List<TempSpliceClosure> GetAll(int uploadId)
        {
            return dATempSpliceClosure.GetAll(uploadId);
        }
        public List<ErrorMessage> InsertSpliceClosureIntoMainTable(UploadSummary summary)
        {
        ...
        private List<ErrorMessage> ParallelInsert(UploadSummary summary)
        {
            ...
            List<TempSpliceClosure> lstTemp = GetAll(summary.id);
            ...
            ParallelLoopResult result = Parallel.ForEach(lstBatch, options, (batchId) =>
            {
                try
                {
                    lock (lockObj)
                    {
                        dATempSpliceClosure.InsertSpliceClosureIntoMainTable(summary, batchId);

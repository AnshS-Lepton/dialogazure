...
{
    public class BLTempCabinet : BLDataUploader
    {

        DATempCabinet daTempObject;

        public BLTempCabinet()
        {
            daTempObject = new DATempCabinet();
            daTempObject.DataUploaderNotificationHandler += NotifyUpdatedStatus;

        }
        public void Save(List<TempCabinet> lstCabinet)
        {
            daTempObject.Save(lstCabinet);
        }

        public List<ErrorMessage> InsertCabinetIntoMainTable(UploadSummary summary)
        {
        ...
        private List<ErrorMessage> ParallelInsert(UploadSummary summary)
        {
            ...
            List<TempCabinet> lstTemp = GetAll(summary.id);
            ...
            ParallelLoopResult result = Parallel.ForEach(lstBatch, options, (batchId) =>
            {
                try
                {
                    lock (lockObj)
                    {
                        daTempObject.InsertCabinetIntoMainTable(summary, batchId);

...
{
    public class BLTempStructure : BLDataUploader
    {
        DATempStructure daTemplate;

        public BLTempStructure()
        {
            daTemplate = new DATempStructure();
        ...
        public List<ErrorMessage> InsertStructureIntoMainTable(UploadSummary summary)
        {
        ...
        private List<ErrorMessage> ParallelInsert(UploadSummary summary)
        {
            ...
            List<TempStructure> lstTemp = GetAll(summary.id);
            ...
            ParallelLoopResult result = Parallel.ForEach(lstBatch, options, (batchId) =>
            {
                try
                {
                    lock (lockObj)
                    {
                        daTemplate.InsertStructureIntoMainTable(summary, batchId);
        ...
        public void Save(List<TempStructure> lstStructure)
        {
            daTemplate.Save(lstStructure);
        }
    }
}

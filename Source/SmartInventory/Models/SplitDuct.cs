using System;
using System.Collections.Generic;
using System.ComponentModel.DataAnnotations;
using System.Linq;
using System.Text;
using System.Threading.Tasks;

namespace Models
{
    public class SplitDuct
    {
        public string split_entity_networkId { get; set; }
        public int split_entity_system_id { get; set; }
        public string split_entity_type { get; set; }
        public string split_entity_network_status { get; set; }
        public int split_duct_system_id { get; set; }

        public string duct_one_network_id { get; set; }
        [Required(ErrorMessageResourceType = typeof(Resources.Resources), ErrorMessageResourceName = "SI_OSP_DUC_NET_FRM_016")]

        public string duct_one_name { get; set; }
        [Required]
        public string duct_one_a_location { get; set; }
        [Required]
        public string duct_one_b_location { get; set; }
        [Required]
        public float? duct_one_measured_length { get; set; }
        [Required]
        public float? duct_one_calculated_length { get; set; }
        public int old_duct_a_system_id { get; set; }
        public string old_duct_a_entity_type { get; set; }

        [Required]
        public string old_duct_a_location { get; set; }
        public int old_duct_b_system_id { get; set; }
        public string old_duct_b_entity_type { get; set; }
        [Required]
        public string old_duct_b_location { get; set; }



        public string duct_two_network_id { get; set; }
        [Required(ErrorMessageResourceType = typeof(Resources.Resources), ErrorMessageResourceName = "SI_OSP_DUC_NET_FRM_016")]
        public string duct_two_name { get; set; }
        [Required]
        public string duct_two_a_location { get; set; }
        [Required]
        public string duct_two_b_location { get; set; }
        [Required]
        public float? duct_two_measured_length { get; set; }
        [Required]
        public float? duct_two_calculated_length { get; set; }
        public int parent_system_id { get; set; }
        public string parent_network_id { get; set; }
        public string parent_entity_type { get; set; }

        public List<EntityDetail> ducts { get; set; }

        public PageMessage objPM { get; set; }
        public int userId { get; set; }

        public string network_status { get; set; }
        public string parent_duct_netwok_id { get; set; }
        public string parent_duct_system_id { get; set; }
        public string splited_by { get; set; }
        public string splitted_on { get; set; }
        public string splitting_system_id { get; set; }
        public string splitting_netwok_id { get; set; }
        public string splitting_entitytype { get; set; }

        public SplitDuct()
        {
            objPM = new PageMessage();
            ducts = new List<EntityDetail>();
          
        }
    }
}
